...
Keep `SKILL.md` bodies well under 500 lines. Prefer far shorter. This repo's craft skills (`mobx`, `react-best-practices`, `commit`) are the length to match.
...
| Spec for a change  | `openspec/changes/`                                | pointer from the task, not AGENTS.md |
